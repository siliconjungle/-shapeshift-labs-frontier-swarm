...
Clean auto-mergeable work that spans multiple known semantic regions is returned as a `rerun` assignment with `retrySlices`, `semanticSliceScopeIds`, and `semanticSliceLeaseKeys`. If admission has already accepted the cross-scope work, the `apply-local` assignment still carries `semanticSliceScopeIds` and `semanticSliceLeaseKeys` so runners can acquire the separate semantic leases before applying. Runners can use those slices to retry or apply same-file work under separate semantic leases instead of promoting the whole file to the root queue. Same-region conflicts still serialize under one local lease, while unknown regions and public-contract/API regions promote to the parent queue for a broader decision.
